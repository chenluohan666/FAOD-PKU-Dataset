...
BBOX_DTYPE = np.dtype({'names': ['t', 'x', 'y', 'w', 'h', 'class_id', 'track_id'],
...
def to_prophesee(loaded_label_list: LOADED_LABELS, yolox_pred_list: YOLOX_PRED_PROCESSED) -> \
    ...
    for loaded_labels, yolox_preds in zip(loaded_label_list, yolox_pred_list):
        ...
        yolox_pred_proph = np.zeros((num_pred,), dtype=BBOX_DTYPE_PRED)
        if num_pred > 0:
            yolox_preds = yolox_preds.detach().cpu().numpy()
            assert yolox_preds.shape == (num_pred, 7)
            yolox_pred_proph['t'] = np.ones((num_pred,), dtype=BBOX_DTYPE_PRED['t']) * time
            yolox_pred_proph['x'] = np.asarray(yolox_preds[:, 0], dtype=BBOX_DTYPE_PRED['x'])
            yolox_pred_proph['y'] = np.asarray(yolox_preds[:, 1], dtype=BBOX_DTYPE_PRED['y'])
            yolox_pred_proph['w'] = np.asarray(yolox_preds[:, 2] - yolox_preds[:, 0], dtype=BBOX_DTYPE_PRED['w'])
            yolox_pred_proph['h'] = np.asarray(yolox_preds[:, 3] - yolox_preds[:, 1], dtype=BBOX_DTYPE_PRED['h'])
            yolox_pred_proph['class_id'] = np.asarray(yolox_preds[:, 6], dtype=BBOX_DTYPE_PRED['class_id'])
            yolox_pred_proph['class_confidence'] = np.asarray(yolox_preds[:, 5], dtype=BBOX_DTYPE_PRED['class_confidence'])
