...
class ObjectLabelFactory(ObjectLabelBase):
    ...
    def from_structured_array(object_labels: np.ndarray,
        ...
        # print('object_labels: ', object_labels.shape)
        ...
        # print('np_labels: ', np_labels.shape)   # [N, 6]
        torch_labels = th.from_numpy(np_labels)
        objframe_idx_2_label_idx = th.from_numpy(objframe_idx_2_label_idx.astype('int64'))
  
        # print(objframe_idx_2_label_idx.numel())
        # print(np.unique(object_labels['t']).size)
        # assert objframe_idx_2_label_idx.numel() == np.unique(object_labels['t']).size # TODO
    ...
    def __getitem__(self, item: int) -> ObjectLabels:
        ...
        assert to_idx > from_idx, 'from_idx:{}, to_dix:{}'.format(from_idx, to_idx)
