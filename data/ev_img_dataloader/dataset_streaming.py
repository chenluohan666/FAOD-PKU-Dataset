...
def get_sequences(path: Path, dataset_config: DictConfig, guarantee_labels: bool) -> List[SequenceForIter]:
    ...
    if dataset_config.name == 'pku_fusion':
        dataset_type = DatasetType.PKU_FUSION
    ...
    time_flip = np.random.rand() < dataset_config.data_augmentation.stream.prob_time_flip
    unpair_prob = dataset_config.data_augmentation.stream.unpair.prob
    min_max_drift = [dataset_config.data_augmentation.stream.unpair.min_drift, 
                     dataset_config.data_augmentation.stream.unpair.max_drift]

    if np.random.rand() < dataset_config.data_augmentation.stream.unpair.prob:
        min_drift = dataset_config.data_augmentation.stream.unpair.min_drift
        max_drift = dataset_config.data_augmentation.stream.unpair.max_drift
        drift = random.randint(min_drift, max_drift)
    else:
        drift = 0

    label_shift = dataset_config.shift.label    # dataset_config.model_type != 'frame'
    image_shift = dataset_config.shift.image
    # print('label_shift in stream: ', label_shift)
