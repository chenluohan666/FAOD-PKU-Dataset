...
class SequenceBase(MapDataPipe):
    ...
    def __init__(self,
        ...
        assert path.is_dir()
        assert dataset_type in {DatasetType.PKU_FUSION, DatasetType.DSEC}, f'{dataset_type} not implemented'
        ...
        # self.image_npy_file = labels_dir / f'images.npy'
        self.image_h5_file = labels_dir / f'images.h5'
    ...
    def _get_labels_from_repr_idx(self, repr_idx: int) -> Optional[ObjectLabels]:
        ...
        if torch.all(self.label_factory[objframe_idx].class_id == 255):
            return None
        
        return self.label_factory[objframe_idx]

    def _get_event_repr_torch(self, start_idx: int, end_idx: int, time_flip=False) -> List[torch.Tensor]:
        ...
        if not time_flip:
            ...
            ev_repr = [torch.flip(x[0], dims=[0]) for x in ev_repr][::-1]

        return ev_repr



    def _get_img_torch(self, start_idx: int, end_idx: int, time_flip=False) -> List[torch.Tensor]:
        # raise NotImplementedError
        assert end_idx > start_idx

        # img_npy = np.load(self.image_npy_file)
        # img = img_npy[start_idx:end_idx]
        with h5py.File(str(self.image_h5_file), 'r') as h5f:
            img = h5f['data'][start_idx:end_idx]
        img = torch.from_numpy(img)

        if img.dtype != torch.uint8:
            img = torch.asarray(img, dtype=torch.float32)
        img = torch.split(img, 1, dim=0)
        # remove first dim that is always 1 due to how torch.split works
        if not time_flip:
            img = [x[0].permute(2, 0, 1) for x in img]  # [C, H, W]
        else:
            img = [torch.flip(x[0].permute(2, 0, 1), dims=[0]) for x in img][::-1]  # [C, H, W]
