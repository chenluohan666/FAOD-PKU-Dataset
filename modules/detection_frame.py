...
class Module(pl.LightningModule):
    ...
    def training_step_with_ssm(self, batch: Any):
        ...
        '''
        ev_tensor_sequence = data[DataType.IMAGE] # squence_length * B * H * W
    ...
    def training_step_with_rnn(self, batch: Any):
        ...
        self.started_training = True
        step = self.trainer.global_step
        ev_tensor_sequence = data[DataType.IMAGE]
        sparse_obj_labels = data[DataType.OBJLABELS_SEQ]
        is_first_sample = data[DataType.IS_FIRST_SAMPLE]
        token_mask_sequence = data.get(DataType.TOKEN_MASK, None)
        ...
        for tidx in range(sequence_len):
            ...
            backbone_features, states = self.mdl.forward_backbone_rnn(ev_input=ev_tensors,
                                                                  previous_states=prev_states,
    ...
    def _val_test_step_impl_with_ssm(self, batch: Any, mode: Mode) -> Optional[STEP_OUTPUT]:
        ...
        backbone_features, states = self.mdl.forward_backbone_ssm(ev_input=ev_tensor_sequence, previous_states=prev_states, train_step=False)
        prev_states = states
    ...
    def _val_test_step_impl_with_rnn(self, batch: Any, mode: Mode) -> Optional[STEP_OUTPUT]:
        ...
        for tidx in range(sequence_len):
            ...
            backbone_features, states = self.mdl.forward_backbone_rnn(ev_input=ev_tensors, previous_states=prev_states)
            prev_states = states
