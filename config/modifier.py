...
def dynamically_modify_train_config(config: DictConfig):
    with open_dict(config):
        ...
        assert dataset_name in {'pku_fusion', 'dsec'}
        dataset_hw = get_dataloading_hw(dataset_config=dataset_cfg)

        mdl_cfg = config.model

        dataset_cfg.model_type = mdl_cfg.backbone.type
        mdl_name = mdl_cfg.name
        if mdl_name == 'rnndet':
            backbone_cfg = mdl_cfg.backbone
            backbone_name = backbone_cfg.name
            if backbone_name == 'single_modal' or backbone_name == 'forward_fusion' or backbone_name == 'overall_fusion':
            ...
            if backbone_cfg.type == 'event' or backbone_cfg.type == 'frame':
                dataset_cfg.data_augmentation.random.unpair.prob = 0
                dataset_cfg.data_augmentation.stream.unpair.prob = 0
        else:
