...
def apply_ssm(
    ...
    if B_bars.ndim == 3:
        ...
        Bu_elements = torch.vmap(lambda B_bar, u: B_bar @ u)(B_bars, cinput_sequence.unsqueeze(-2))
    else:
        # Static timesteps
...
def discretize_bilinear(Lambda, B_tilde, Delta):
    ...
    Args:
        ...
        B_tilde (complex64): input matrix                      (P, H),      [L, P, 1]
        Delta (float32): discretization step sizes             (P,)         [L, P]
    Returns:
        discretized Lambda_bar (complex64), B_bar (complex64)  (P,), (P,H)
    """
    Lambda = torch.view_as_complex(Lambda)

    Identity = torch.ones(Lambda.shape[0], device=Lambda.device)    # [P]
    BL = 1 / (Identity - (Delta / 2.0) * Lambda)                    # [L, P]
    Lambda_bar = BL * (Identity + (Delta / 2.0) * Lambda)
    B_bar = (BL * Delta)[..., None] * B_tilde

    Lambda_bar = torch.view_as_real(Lambda_bar)
    B_bar = torch.view_as_real(B_bar)   # [L, P, 2]
...
#     Lambda_bar = torch.exp(Lambda * Delta)      # [L, P]
...
class S5SSM(torch.nn.Module):
    def __init__(
        ...
        # self.target_size = [5, 6]
        # assert size[0] % self.target_size[0] == 0 and size[1] % self.target_size[1] == 0
        # kernel_size_H = size[0] // self.target_size[0]
        # kernel_size_W = size[1] // self.target_size[1]
        # stride_H = kernel_size_H
        # stride_W = kernel_size_W
        # self.max_pool = nn.MaxPool2d(kernel_size=(kernel_size_H, kernel_size_W), stride=(stride_H, stride_W))
        self.h, self.p = h, p

        self.x_proj = nn.Linear(
            h, (h+p)*2+1, bias=False
        )
        self.dt_proj = nn.Linear(1, h, bias=True)
...
#             )
    ...
    def initial_state(self, batch_size: Optional[int]):
        ...
        _, C_tilde = self.get_BC_tilde(torch.zeros(self.h, 1, 2), torch.zeros(1, self.p, 2))

        return torch.zeros((*batch_shape, C_tilde.shape[-2]))

    def get_BC_tilde(self, B, C):
        match self.bcInit:
            case "dense_columns" | "dense" | "complex_normal":
                B_tilde = as_complex(B)
                C_tilde = C
            case "factorized":
                raise NotImplementedError
...
#         return y, x
    ...
    def forward(self, signal, prev_state, step_scale: float | torch.Tensor = 1.0):
        '''
        signal: [B*H*W, L, D], [L, D]
        '''

        dt_B_C = self.x_proj(signal)
        dt, B, C = torch.split(dt_B_C, [1, self.h*2, self.p*2], dim=-1)
        dt = (self.dt_proj.weight @ dt.t()).permute(1, 0)   # [L, h]

        L = B.shape[0]
        B_ = B.view(L, self.h, 1, 2).permute(0, 3, 1, 2)
        C_ = C.view(L, self.p, 1, 2).permute(0, 3, 1, 2)

        B_ = torch.matmul(B_, B_.permute(0, 1, 3, 2)).permute(0, 2, 3, 1)
        C_ = torch.matmul(C_, C_.permute(0, 1, 3, 2)).permute(0, 2, 3, 1)

        

        B_tilde, C_tilde = self.get_BC_tilde(B_, C_)
        ...
        # if not torch.is_tensor(step_scale) or step_scale.ndim == 0:
        #     # step = step_scale * torch.exp(self.log_step)
        #     setp = step_scale * torch.exp(dt)
        # else:
        #     # TODO: This is very expensive due to individual steps being multiplied by B_tilde in self.discretize
        #     step = step_scale[:, None] * torch.exp(self.log_step)   # [L, h]
        #     step = step_scale[:, None] * torch.exp(self.log_step)   # [L, h]
        step = step_scale * torch.exp(dt)
