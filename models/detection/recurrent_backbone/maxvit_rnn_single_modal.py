...
class RNNDetector(BaseDetector):
    def __init__(self, mdl_config: DictConfig):
        ...
        ###### Config ######
        if mdl_config.type == 'event':
            in_channels = mdl_config.ev_input_channels
        elif mdl_config.type == 'frame':
            in_channels = mdl_config.img_input_channels
    ...
    def forward(self, x: th.Tensor, prev_states: Optional[LstmStates] = None, token_mask: Optional[th.Tensor] = None, train_step: bool = True,) \
        ...
        for stage_idx, stage in enumerate(self.stages):
            x, state = stage(x, prev_states[stage_idx], token_mask if stage_idx == 0 else None, train_step)
            states.append(state)
