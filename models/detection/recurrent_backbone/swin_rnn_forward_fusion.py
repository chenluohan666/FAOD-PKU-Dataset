...
from models.layers.swins.swins import (SwinTransformerBlock)
...
class RNNDetector(BaseDetector):
    def __init__(self, mdl_config: DictConfig):
        ...
        input_in_res_hw = mdl_config.in_res_hw
        ...
        input_in_res_hw = [int(item / 4) for item in input_in_res_hw]
        ...
        initial_stage = RNNFusionDetectorStage(dim_in_ev=ev_in_channels,
                                               ...
                                               stage_dim=stage_dim,
                                               input_resolution = input_in_res_hw,
        ...
        for stage_idx, (num_blocks, T_max_chrono_init_stage) in \
            ...
            if stage_idx == 0:
                continue
            input_in_res_hw =  [int(item / 2) for item in input_in_res_hw]
...
class SwinAttentionPairCl(nn.Module):
    def __init__(self,
                 dim: int,
        ...
        self.att_window1 = SwinTransformerBlock(dim=dim,
                                               input_resolution = input_resolution,                                     
                                               attention_cfg=attention_cfg)
        
        self.att_window2 = SwinTransformerBlock(dim=dim,
                                               input_resolution = input_resolution,                                     
                                               attention_cfg=attention_cfg)

    def forward(self, x):
        x = self.att_window1(x)
        x = self.att_window2(x)
        return x
...
class RNNFusionDetectorStage(nn.Module):
    ...
    def __init__(self,
        ...
        ev_blocks = [SwinAttentionPairCl(dim=stage_dim,
                                        input_resolution = input_resolution,
                                        attention_cfg=attention_cfg) for i in range(num_blocks)]

        img_blocks = [SwinAttentionPairCl(dim=stage_dim,
                                        input_resolution = input_resolution,
                                        attention_cfg=attention_cfg) for i in range(num_blocks)]
    ...
    def forward_rnn(self, ev_input: th.Tensor,
        ...
        height, width = self.input_resolution[0], self.input_resolution[1]
        ev_input = self.ev_downsample_cf2cl(ev_input)  # N C H W -> N H W C
        img_input = self.img_downsample_cf2cl(img_input)  # N C H W -> N H W C

        ev_input = rearrange(ev_input, "N H W C -> N (H W) C")
        img_input = rearrange(img_input, "N H W C -> N (H W) C")
        if token_mask is not None:
            assert self.mask_token is not None, 'No mask token present in this stage'
            ev_input[token_mask] = self.mask_token
            img_input[token_mask] = self.mask_token
        ...
        ev_input = rearrange(ev_input, "N (H W) C -> N H W C", H = height, W = width)
        img_input = rearrange(img_input, "N (H W) C -> N H W C", H = height, W = width)
...
class RNNDetectorStage(nn.Module):
    ...
    def __init__(self,
        ...
        self.downsample_cf2cl = get_downsample_layer_Cf2Cl(dim_in=dim_in,
                                                           ...
                                                           downsample_factor=spatial_downsample_factor,
                                                           downsample_cfg=downsample_cfg)
        blocks = [SwinAttentionPairCl(dim=stage_dim,
                                        input_resolution = input_resolution,
                                        attention_cfg=attention_cfg) for i in range(num_blocks)]
    ...
    def forward_rnn(self, x: th.Tensor,
        ...
        x = rearrange(x, "N (H W) C -> N H W C", H = height, W = width)
