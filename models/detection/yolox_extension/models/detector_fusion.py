...
class YoloXDetector(th.nn.Module):
    ...
    def forward_backbone_rnn(self,
        ...
        with CudaTimer(device=ev_input.device, timer_name="Backbone"):
            backbone_features, states, loss_align, feature_alighed, feature_unalighed = self.backbone(ev_input, img_input, previous_states, token_mask)
        if self.training:
            return backbone_features, states, loss_align, feature_alighed, feature_unalighed
        else:
    ...
    def forward_backbone_ssm(self,
            ...
            Tuple[BackboneFeatures, LstmStates]:
        # print('forward_backbone_ssm drift: ', drift)
        with CudaTimer(device=ev_input.device, timer_name="Backbone"):
            backbone_features, states, loss_align, feature_alighed, feature_unalighed = self.backbone(ev_input, img_input, previous_states, token_mask, train_step, drift)
        if self.training:
            return backbone_features, states, loss_align
        else:
            return backbone_features, states           
